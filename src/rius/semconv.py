...
def kind_attributes(
    kind: SpanKind, name: str | None = None, data_source_id: str | None = None
) -> dict[str, str]:
    ...
    (``gen_ai.data_source.id``); only the caller knows it, so it is an
    ...
    if kind is SpanKind.RETRIEVER and data_source_id is not None:
